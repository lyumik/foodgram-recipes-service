...
class RecipeListCreateView(generics.ListCreateAPIView):
    queryset = Recipe.objects.all().prefetch_related("tags", "recipe_ingredients__ingredient")
...
class RecipeDetailView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Recipe.objects.all().prefetch_related("tags", "recipe_ingredients__ingredient").select_related()
...
class ShoppingCartView(APIView):
    ...
    def post(self, request, recipe_id):
        ...
        item, created = ShoppingCart.objects.get_or_create(user_id=user_id, recipe=recipe)
        if not created:
    ...
    def delete(self, request, recipe_id):
        user_id = request.data.get("user_id", 0)
        item = generics.get_object_or_404(ShoppingCart, user_id=user_id, recipe_id=recipe_id)
        item.delete()
...
class ShoppingCartDownloadView(APIView):
    def get(self, request):
        ...
        user_id = request.query_params.get("user_id", 0)
        items = ShoppingCart.objects.filter(user_id=user_id).select_related("recipe")
